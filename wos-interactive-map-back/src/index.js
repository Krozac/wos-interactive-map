...
app.use((req, res, next) => {
  ...
  if (process.env.PROD === 'true') { // Enforce host header in production
    const host = req.headers.host;

    if (!host || !host.includes(allowedHost)) {
        const redirectUrl = `https://${allowedHost}${req.url}`;
...
app.post('/validate-id', async (req, res) => {
  ...
  try {
    ...
    if (apiResponse.code === 0 && apiResponse.msg === 'success') {
      ...
      tokens[token] = { valid: true, expiry: Date.now() + 3600000, fid: apiResponse.data.fid };
...
app.listen(PORT, () => {
  console.log(`✅ Server is running on ${allowedHost}:${PORT}`);
});
